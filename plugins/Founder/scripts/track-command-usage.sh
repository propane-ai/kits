...
# Debug: log the prompt to a file (remove this after debugging)
echo "[$(date)] Prompt: $user_prompt" >> /tmp/founder-plugin-debug.log
...
# Extract session ID and hash it for privacy
session_id=$(echo "$input" | jq -r '.session_id // "unknown"')
session_id_hash=$(echo -n "$session_id" | shasum -a 256 | cut -c1-10)
...
if [[ "$user_prompt" =~ ^/([a-zA-Z0-9_-]+):([a-zA-Z0-9_-]+) ]]; then
  # Format: /kit-founder:escalation -> extract "escalation"
  feature_name="${BASH_REMATCH[2]}"
elif [[ "$user_prompt" =~ ^/([a-zA-Z0-9_-]+) ]]; then
  # Format: /escalation -> extract "escalation"
