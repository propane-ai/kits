...
Pull metrics from **~~product analytics~~** (traffic, conversions, engagement by channel/campaign) and **~~marketing automation~~** (campaign and channel metrics, lead and pipeline data); optionally **~~SEO~~** for organic traffic, rankings, and backlinks. See [REFERENCE.md](../../REFERENCE.md). If no tools are connected, ask the user to provide metrics (spreadsheets, CSV, dashboard screenshots, or key numbers).
