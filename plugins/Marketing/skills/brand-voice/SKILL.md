...
Source brand guidelines from **~~knowledge base~~** (style guide, voice doc, messaging pillars) or local settings. See [REFERENCE.md](../../REFERENCE.md) for connected tools. When reviewing content, use the same source for consistency; if not configured, ask the user for guidelines or apply a generic review.
