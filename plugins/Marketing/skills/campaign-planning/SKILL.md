...
Pull past campaign performance and benchmarks from **~~marketing automation~~** and **~~product analytics~~** when connected. Use **~~knowledge base~~** for campaign playbooks, briefs, and brand guidelines. See [REFERENCE.md](../../REFERENCE.md). If a tool is not connected, use what the user provides and note what would improve the plan (e.g. "Historical benchmarks would strengthen the success metrics section").
