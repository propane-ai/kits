...
User runs `/plan-campaign` or asks to plan, design, or build a marketing campaign.
...
- **~~knowledge base** (if connected):** Campaign playbooks, briefs, past campaign docs, brand guidelines. See [REFERENCE.md](../REFERENCE.md).
- **~~marketing automation** (if connected):** Past campaign performance, channel benchmarks, audience segments.
- **~~product analytics** (if connected):** Historical performance benchmarks to inform targets and KPIs.
- If a tool is not connected, use what the user provides and note what would improve the brief (e.g. "Historical benchmarks would strengthen the success metrics section").
