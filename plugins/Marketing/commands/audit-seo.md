...
User runs `/audit-seo` or asks for an SEO audit, keyword research, content gap analysis, technical SEO check, or competitor SEO comparison.
...
- **~~SEO** (if connected):** Keyword data, search volume, difficulty, ranking positions, backlink profiles. See [REFERENCE.md](../REFERENCE.md).
- **~~product analytics** (if connected):** Cross-reference keyword targets with organic traffic and conversions.
- If no tools are connected, use web search for the keyword landscape and note: "For precise volume and difficulty data, connect an SEO tool via MCP per REFERENCE.md."
