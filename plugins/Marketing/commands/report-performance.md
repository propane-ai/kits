...
User runs `/report-performance` or asks for a marketing report, performance analysis, campaign results, or metrics summary.
...
- **~~product analytics** (if connected):** Pull performance data automatically (traffic, conversions, engagement by channel/campaign). See [REFERENCE.md](../REFERENCE.md).
- **~~marketing automation** (if connected):** Campaign and channel metrics, lead and pipeline data.
- **~~SEO** (if connected):** Organic traffic, keyword rankings, backlinks for SEO/owned channel reporting.
- If no tools are connected, ask the user to provide metrics (spreadsheets, CSV, dashboard screenshots described in text, or key numbers).
