...
User runs `/review-brand` or asks to review, check, or audit content against brand guidelines.
...
- **~~knowledge base** (if connected):** Style guide, brand voice doc, messaging pillars, terminology list. See [REFERENCE.md](../REFERENCE.md).
- If no tool is connected, use local settings or user-provided guidelines; note "For consistent reviews across content, add a style guide to ~~knowledge base~~ or local settings."
