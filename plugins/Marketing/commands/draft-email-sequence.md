...
User runs `/draft-email-sequence` or asks to create, design, build, or draft an email sequence, drip campaign, nurture flow, or onboarding series.
...
- **~~email marketing** (if connected):** Set up sequence as flow/automation; use platform features (smart send time, splits, A/B tests). See [REFERENCE.md](../REFERENCE.md).
- **~~marketing automation** (if connected):** Lead scoring, lifecycle stage, enrollment triggers for segmentation and exit conditions.
...
### If ~~marketing automation is connected (e.g., HubSpot, Marketo)
