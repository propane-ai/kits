...
When building a PRD or feature spec, pull from connected tools when available:
...
- **~~knowledge base~~**: Prior specs, design docs, research documents, meeting notes, decision records
- **~~design~~**: Related mockups, wireframes, design explorations; design system components relevant to the feature

If a tool is not connected, work from what the user provides; do not ask them to connect tools. Note "Context limited to user input" if no tools are available.
